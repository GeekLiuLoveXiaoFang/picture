...
class Nude(object): #创建Nude类

    skin = namedtuple("Skin", "id skin region x y") #利用collections.namedtuple创建一个Nude类

    def __init__(self, path_or_image): #定义Nude类的初始化方法
        #若path_or_image为Image.Image类型的实例，直接赋值
        if isinstance(path_or_image, Image.Image):
            self.image = path_or_image
        #若path_or_iamge为str类型的实例，打开图片
        elif isinstance(path_or_image, str):
            self.image = Image.open(path_or_image)

        #获得图片所有颜色通道
        bands = self.image.getbands()
        #判断是否为单通道图片（即灰度图），是则转为RGB图
        if len(bands) == 1:
            #新建相同大小的RGB图像
        ...
        #这些元素中的区域号代表的区域都是等待合并的区域
        self.merge_regions = []
        #整合后皮肤区域，元素的索引即为皮肤区域号，元素都是包含一些Skin对象的列表
        self.skin_regions = []
    ...
    def resize(self, maxwidth=1000, maxheight=1000):
        ...
        if maxheight:
            if self.height > maxheight:
                ...
                wsize = int((float(self.height) * float(hpercent)))
                fname = self.image.filename
    ...
    def parse(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                #给每个像素分配唯一的id值（1,2,3，。。。height*width)
                #x,y的值从0开始
